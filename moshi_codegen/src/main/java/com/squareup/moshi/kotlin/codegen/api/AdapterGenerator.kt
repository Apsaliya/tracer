...
private const val MOSHI_UTIL_PACKAGE = "com.squareup.moshi.internal.Util"
...
internal class AdapterGenerator(
    ...
    private val composedProperties: List<Pair<TargetProperty, PropertyGenerator>>
) {
  ...
  fun prepare(messager: Messager?, generateProguardRules: Boolean, typeHook: (TypeSpec) -> TypeSpec = { it }): PreparedAdapter {
    for (property in nonTransientProperties) {
      property.allocateNames(nameAllocator)
    }

    for (property in composedProperties.map { it.second }) {
      property.allocateNames(nameAllocator)
    }

    val generatedAdapter = generateType(messager).let(typeHook)
    val result = FileSpec.builder(className.packageName, adapterName)
    result.addFileComment("Code generated by moshi-kotlin-codegen. Do not edit.", arrayOf<Any>())
  ...
  private fun generateType(messager: Messager?): TypeSpec {
    ...
    for (uniqueAdapter in ((nonTransientProperties + composedProperties.map { it.second }).distinctBy { it.delegateKey })) {
      result.addProperty(
    ...
    //DO NOT REMOVE
    /*try {
      for (uniqueAdapter in composedProperties.map { it.second }.distinctBy { it.delegateKey }) {
        try {
          result.addProperty(
              uniqueAdapter.delegateKey.generateProperty(
                  nameAllocator, typeRenderer, moshiParam, uniqueAdapter.name
          ...
          //messager.printMessage(Diagnostic.Kind.MANDATORY_WARNING, "AlreadyExistsException caught")
        } catch (e: Exception) {
          throw e
        } finally {
          val qualifierNames = uniqueAdapter.delegateKey.jsonQualifiers.joinToString("") {
            "At${it.className.simpleName}"
          }
          val adapterName = "${uniqueAdapter.delegateKey.type.toVariableName().decapitalize()}${qualifierNames}Adapter"

          //messager.printMessage(Diagnostic.Kind.MANDATORY_WARNING, "ADAPTER SHIT ${adapterName}")
        }
      }
    } catch (e: IllegalArgumentException) {
      e.printStackTrace()
      *//*composedProperties.forEach {
        messager.printMessage(Diagnostic.Kind.MANDATORY_WARNING, " ${it.second.name}")
      }*//*
      throw e
    }*/
  ...
  private fun generateFromJsonFun(classBuilder: TypeSpec.Builder): FunSpec {
    ...
    for (property in nonTransientProperties) {
      // If property is composed, we don't always have to make is nullable
      // Reason being we are looking it up from another JsonAdapter and reading from JsonReader
      // We make it nullable only if it is not required
      // In cases where fromJson returns null for Composed property, we anyway throw an exception down below
      // All the properties that are not composed will always be nullable since we are reading them by iterating over JsonReader.
      val nullable = if (property.isComposed) !property.isRequired else true
      result.addCode("%L", property.generateLocalProperty(nullable))
    ...
    val composedProperties = propertyList.filter { it.isComposed }
    var counter = 0
    composedProperties.forEach {
      result.addStatement("val peekJson${counter} = %N.peekJson()", readerParam)
      if (!it.delegateKey.nullable) {
        val peekJsonParam = ParameterSpec.builder(
            nameAllocator.newName("peekJson${counter}"),
            JsonReader::class
        ).build()
        val exception = unexpectedNull(it, peekJsonParam)
        result.addStatement(
            "%N = %N.fromJson(peekJson${counter}) ?: throw·%L",
            it.localName, nameAllocator[it.delegateKey], exception
        )
      } else {
        result.addStatement(
            "%N = %N.fromJson(peekJson${counter})",
            it.localName, nameAllocator[it.delegateKey]
        )
      }
      counter++
    }
    ...
    for (input in components) {
      if (input is ParameterOnly ||
          (input is ParameterProperty && input.property.isTransient) || (input is ParameterProperty && input.property.isComposed)) {
        updateMaskIndexes()
    ...
    for (input in components.filterIsInstance<ParameterComponent>()) {
      ...
      if (input is PropertyComponent) {
        val property = input.property
        // We don't want to throw for composed properties because their type (required or optional) is already decided above
        // So we don't have to throw exception for required composed properties here.
        if (!property.isTransient && property.isRequired && !property.isComposed) {
          result.addMissingPropertyCheck(property, readerParam)
        }
      }
      separator = ",\n"
  ...
  private fun generateToJsonFun(): FunSpec {
    ...
    (nonTransientProperties.filter { !it.isComposed }).forEach { property ->
    ...
    composedProperties.forEach { property ->
      if (property.first.type.isNullable) {
        result.addStatement("%N.name(%S)", writerParam, property.second.jsonName)
        result.addStatement("%N.toJson(%N, %N.${property.first}?.%N)",
            nameAllocator[property.second.delegateKey], writerParam, valueParam, property.second.name)
      } else {
        result.addStatement("%N.name(%S)", writerParam, property.second.jsonName)
        result.addStatement("%N.toJson(%N, %N.${property.first}.%N)",
            nameAllocator[property.second.delegateKey], writerParam, valueParam, property.second.name)
